...
from number_predictor import predictor
...
def plot_one_box(t,x, img, color=None, label=None, line_thickness=None):
    ...
    if label[0] == 'n':
        plate = img[c1[1]:c2[1],c1[0]:c2[0]]
        t += 1
        plate = cv2.cvtColor(plate, cv2.COLOR_BGR2GRAY)[:, :, np.newaxis]
        label = predictor(plate)
        label = re.sub('\[UNK\]', '', label)
        color = [255,0,0]
    ...
    #cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
        ...
        print(plate.shape)
        tl = plate.shape[1] * 0.004 + 0.188
        t_size = cv2.getTextSize(label, 0, fontScale=tl , thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        # cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 5, [225, 255, 255], thickness=1, lineType=cv2.LINE_AA)
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl, [225, 255, 255], thickness=1, lineType=cv2.LINE_AA)
